...
class POINT:
    ...
    def lancar_chamada(self):
        self.frame_principal = ttk.Frame(self.janela, padding=30)
        self.frame_principal.pack(fill=tk.BOTH, expand=True)

        Entrada_frame = ttk.Frame(self.frame_principal,padding=4)
        ...
        # FUNÇÃO: GERAR CÓDIGO ALEATÓRIO PARA A CHAMADA, O CÓDIGO É GERADO COM 2 NÚMEROS, 2 LETRAS E MAIS 2 NÚMEROS. EXEMPLO: 12AB34    
        ...
        def gerar_codigo():
            ...
            self.Entrada_tarefa.insert(0, codigo)
        ...
        self.tree = ttk.Treeview(self.frame_principal,columns=("Data","R.A.","Aluno", "Situação","Curso"),show="headings",selectmode="browse")
...
# FUNÇÃO: MARCAR PRESENÇA MANUALMENTE CASO O ALUNO NÃO TENHA ACESSO E ALGUM MODO AO SISTEMA. 
    ...
    def marcar_presenca(self):
        ra = tk.simpledialog.askstring("Entrada", "Digite o R.A. do aluno:")
        nome = tk.simpledialog.askstring("Entrada", "Digite o nome do aluno:")
        curso = tk.simpledialog.askstring("Entrada", "Digite o curso do aluno:")
        situacao = self.prioridade.get()
        data = self.entrada_data.get()

        if not ra or not nome or not curso or not situacao or not data:
            messagebox.showerror("Erro", "Todos os campos devem ser preenchidos!")
            return

        self.tree.insert("", tk.END, values=(data, ra, nome, situacao, curso))
        messagebox.showinfo("Sucesso", "Presença marcada com sucesso!")

        ttk.Button(self.frame_principal, text="Marcar Presença Manual", command=self.marcar_presenca).pack(pady=10)
